    ...
    <section id="testimonials" className="py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">Client voice</p>
            <h2 className="font-display text-3xl font-semibold tracking-tight md:text-4xl">
              Partners who stay because we deliver.
            </h2>
            <p className="mt-3 max-w-2xl text-lg text-muted-foreground">
              We build long-term relationships by shipping the work, not just the decks.
            </p>
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((item, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="relative overflow-hidden rounded-2xl border border-border/60 bg-card/90 p-6 shadow-lg shadow-primary/10"
            >
