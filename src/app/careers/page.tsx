    ...
    <div className="relative min-h-screen bg-linear-to-b from-background via-background to-muted/40">
      ...
      <main className="mx-auto max-w-6xl px-4 py-16 md:py-24">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full border-2 border-gray-600 bg-primary/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary dark:border-gray-500">
            ...
            <p className="mt-3 max-w-2xl text-lg text-muted-foreground">
              We’re engineers, designers, and builders who care about fast
              delivery, great craft, and measurable outcomes. Show us what you
              can build, and we’ll open the right door for you.
            </p>
          </div>
        ...
        <div id="roles" className="mt-12 grid gap-6 md:grid-cols-2">
          {careers.map((role) => (
            <Card
              key={role.slug}
              className="group h-full border-2 border-gray-600 bg-card/90 shadow-sm shadow-primary/10 transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-lg dark:border-gray-500"
            >
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                  {role.responsibilities.slice(0, 2).map((item) => (
                    <span
                      key={item}
                      className="rounded-full border-2 border-gray-600 px-3 py-1 transition-colors group-hover:border-primary/40 group-hover:text-foreground dark:border-gray-500"
                    >
                      {item}
                    </span>
                  ))}
                ...
                <Button
                  asChild
                  variant="outline"
                  className="group-hover:border-2 group-hover:border-gray-600 dark:group-hover:border-gray-500 border-2 border-gray-600 dark:border-gray-500"
                >
                  <Link
                    href={`/careers/${role.slug}`}
